...
tune_xgboost.py
===============
Kịch bản tối ưu hóa siêu tham số (Hyperparameter Tuning) bằng Optuna cho XGBoost.
...
- Test: 2025-10-01 đến 2026-04-06
"""
...
# Tự động phát hiện GPU để tăng tốc huấn luyện tối đa trên Kaggle
try:
    import torch
    DEVICE_NAME = "cuda" if torch.cuda.is_available() else "cpu"
    logger.info(f"🖥️ Thiết bị huấn luyện tự động phát hiện: {DEVICE_NAME.upper()}")
except ImportError:
    DEVICE_NAME = "cpu"
    logger.info("🖥️ Không import được torch. Sử dụng thiết bị mặc định: CPU")
...
def tune_gas(df_feat, target_gas, n_trials=30):
    logger.info(f"🚀 Bắt đầu tối ưu hóa siêu tham số XGBoost cho: {target_gas} với {n_trials} trials")
    ...
    # 5. Lưu bộ siêu tham số tốt nhất vào models/xgboost/
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Tự động tìm kiếm siêu tham số bằng Optuna cho XGBoost.")
